document.addEventListener('DOMContentLoaded', function () {
    ...
    // Savings
    // savingsAccounts: { name, balance, goal (optional) }
    let savingsAccounts = [];
    // savingsContributions: { accountName, amount, frequency, startDate }
    ...
    function updateDisplay() {
      ...
      // 2. Calculate running totals for Checking
      ...
      // 9. Render Savings Contributions dropdown
    ...
    function calculateRunningTotals() {
      ...
      while (currentDate <= endDate) {
        ...
        if (adjustment) {
          // Overwrite dailyNet/balance if there's an adjustment amount
          ...
          // If there's an adjustment event, REPLACE the entire event description
          if (adjustment.event) {
    ...
    function deleteEntry(event) {
      ...
      if (type === 'bill') {
        ...
        // Remove any contributions for that account
